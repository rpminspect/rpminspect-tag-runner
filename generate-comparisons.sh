...
for after_build in $(cat logs/*.success); do
    ...
    while true; do
        ...
        if [[ -z "${package}" ]]; then
            echo "ERROR: NVR to package check failed on ${after_build} - sleeping"
            sleep 60
	    continue
        fi
        echo "INFO: Found package ${package}"
	break
    done
    ...
    if [[ -z "$builds" ]]; then
	echo "ERROR: package to NVRs logic broke on $package - please investigate."
	exit 1
    fi
    echo "INFO: Found $builds"
    # If we find one that's older, that's our comparison
    for before_build in $builds; do
	if [ "${before_build}" != "${after_build}" ]; then
		echo "SUCCESS: ${before_build} ${after_build}"
		echo "${before_build} ${after_build}" >> comparison-list.txt
		break
	fi
	echo "INFO: ${before_build} and ${after_build} match"
    done
